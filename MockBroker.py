...
class MockBroker:
    ...
    def buy(self, stock, price, qty):
        if self.Balance >= price * int(abs(qty)):
            ...
            self.holdingsprofitat.update({stock: org_profit-(price*int(abs(qty)))})
            self.Balance -= price * int(abs(qty))
    ...
    def shortBuy(self, stock, price, qty):
        if self.Balance >= price * int(abs(qty)):
            org_qty = int(self.holdings.get(stock, 0))
            self.holdings.update({stock: org_qty + int(qty)})
            org_profit = int(self.holdingsprofitat.get(stock,0))
    ...
    def shortSell(self, stock, price, qty):
        org_qty = int(self.holdings.get(stock, 0))
        org_profit = int(self.holdingsprofitat.get(stock, 0))
        # if org_qty >= int(qty):
        self.holdings.update({stock: org_qty - int(qty)})
    ...
    def sell_off(self, i):
        for key, value in self.holdings.items():
            if value>0:
                ...
                self.shortSell(key,self.price[key][i], value)
