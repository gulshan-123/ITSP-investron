...
Y_train = np.genfromtxt('data/Training_data_OUTPUT.csv', delimiter=',')
X_val = np.genfromtxt('data/cross_validation_sliced.csv', delimiter=',')
y_val = np.genfromtxt('data/cross_validation_OUTPUT.csv', delimiter=',')
X_test = np.genfromtxt('data/test_set_sliced.csv', delimiter=',')
y_test = np.genfromtxt('data/test_set_OUTPUT.csv', delimiter=',')
...
# Assuming each sample is treated as a single timestep with multiple features
X_n = tf.reshape(X_n, (X_n.shape[0], 1, X_n.shape[1]))
...
model = Sequential(
    [
        ...
        Input(shape=(1, 9)),  # input shape for LSTM: (timesteps, features)
        LSTM(64, activation='relu', name="LSTM_1", return_sequences=True),
...
history = model.fit(X_n, Y_train, validation_data=(X_val_n, y_val), epochs=100, batch_size=32)
